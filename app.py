...
# 🧠 第三部分：全維度報告生成
# ==========================================
def generate_pro_report(df, res_score, rsi, f, s, r, cost_input):
    ...
    report += f"- **關鍵區間**：壓力 **{r1:.2f}** / 支撐 **{s1:.2f}**。目前 RSI 為 **{rsi:.1f}**。\n"
    
    if cost_input > 0:
        report += "\n> 🎯 **操作策略**："
        if last_p < s1: report += "股價已破支撐，建議降低部位避險。"
        elif res_score >= 2 and f > 50: report += "趨勢及籌碼結構健全，建議持股續抱。"
        else: report += "盤勢震盪，守住移動停利線即可。"
    return report
...
if stock_input:
    with st.spinner('掃描全維度數據中...'):
        ...
        if df is not None:
            ...
            # 軌跡與新聞
            st.subheader("📅 歷史量化數據")
            st.table(source[['日期', '收盤價', 'MACD', 'RSI']].tail(5))
