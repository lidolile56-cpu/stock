# 檔名：20150424 MACD + RSI 莫蘭迪美學觸控版 (左側防遮擋優化).py
...
if stock_input:
    ...
    if d_data:
        ...
        # 💡 核心修改區塊：將所有的 align 設為 'right'，dx 設為負值 (-10)
        ...
        # 向左對齊位移
        text_date_p = line_price.mark_text(align='right', dx=-10, dy=-15, fontSize=12, fontWeight='bold', color=morandi_yellow).encode(text='日期:N').transform_filter(nearest)
        text_price = line_price.mark_text(align='right', dx=-10, dy=5, fontSize=14, fontWeight='bold', color=morandi_yellow).encode(text=alt.Text('收盤價:Q', format='.2f')).transform_filter(nearest)
        ...
        # 向左對齊位移
        text_date_m = alt.Chart(source).mark_text(align='right', dx=-10, dy=-15, fontSize=12, fontWeight='bold', color=morandi_yellow).encode(x=x_axis, y=alt.Y('MACD柱狀'), text='日期:N').transform_filter(nearest)
        text_macd = alt.Chart(source).mark_text(align='right', dx=-10, dy=5, fontSize=14, fontWeight='bold', color=morandi_yellow).encode(x=x_axis, y=alt.Y('MACD柱狀'), text=alt.Text('MACD柱狀:Q', format='.3f')).transform_filter(nearest)
        ...
        # 向左對齊位移
        text_date_r = line_rsi.mark_text(align='right', dx=-10, dy=-15, fontSize=12, fontWeight='bold', color=morandi_yellow).encode(text='日期:N').transform_filter(nearest)
        text_rsi = line_rsi.mark_text(align='right', dx=-10, dy=5, fontSize=14, fontWeight='bold', color=morandi_yellow).encode(text=alt.Text('RSI:Q', format='.1f')).transform_filter(nearest)
