# 檔名：20150424 MACD + RSI 終極互動與深度診斷版.py
...
if stock_input:
    ...
    if d_data:
        ...
        # 動態文字：日期與價格
        text_date = line_price.mark_text(align='left', dx=10, dy=-15, fontSize=12, color='gray').encode(text='日期:N').transform_filter(nearest)
        text_price = line_price.mark_text(align='left', dx=10, dy=5, fontSize=14, fontWeight='bold', color='#1f77b4').encode(text=alt.Text('收盤價:Q', format='.2f')).transform_filter(nearest)
        c_price = (line_price + selectors + rules + points_price + text_date + text_price).properties(height=200, title="股價走勢")
        ...
        text_macd = alt.Chart(source).mark_text(align='left', dx=5, dy=-10, fontSize=13, fontWeight='bold').encode(
            x=x_axis, y=alt.Y('MACD柱狀'), text=alt.Text('MACD柱狀:Q', format='.3f'),
            color=alt.condition(alt.datum['MACD柱狀'] > 0, alt.value('#ff4b4b'), alt.value('#00cc96'))
        ).transform_filter(nearest)
        c_macd = (bar_macd + rules + text_macd).properties(height=150, title="MACD 動能")
        ...
        text_rsi = line_rsi.mark_text(align='left', dx=10, dy=-10, fontSize=13, fontWeight='bold', color='#9467bd').encode(text=alt.Text('RSI:Q', format='.1f')).transform_filter(nearest)
        c_rsi = (line_rsi + rules + points_rsi + text_rsi).properties(height=150, title="RSI (14)")
